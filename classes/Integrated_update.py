...
class Case:
    def __init__(self, president, save_mode=False, analysis_mode=True):
        ...
        *** 3.12 下载link列中的link并生成content列 （还没写）
        ...
        # current用来储存最新的结果
        try:
            self.current = pd.read_excel('Processed/' + self.president + '/' + self.president + '_' + str(self.latest()) + '.xlsx', index_col=0)
        except:
        ...
        if analysis_mode == True:
            ...
            try:
                self.iDcN = pd.read_excel('Analysis/' + self.president + '/' + self.president + '_' + 'travels_index_DATE_columns_Names_cell_Countries.xlsx',
                                          index_col=0)
                self.iDcC = pd.read_excel('Analysis/' + self.president + '/' + self.president + '_' + 'travels_index_DATE_columns_Countries_cell_Names.xlsx',
                                          index_col=0)
                self.iCcN = pd.read_excel('Analysis/' + self.president + '/' + self.president + '_' + 'travels_index_Countries_columns_Names_cell_Days.xlsx',
                                          index_col=0)
            except:
                self.iDcN = None
                self.iDcC = None
    ...
    def add_travel_meet_kind(self):
        ...
        travel = ['travels', 'travel', 'travels to', 'travel to', 'tours ', 'tour ', 'visits ', 'visit', 'accompanies ', 'accompanied ', 'follows ', 'follow ',
                  'en route ']
        meet = ['meets ', 'meet ', 'meetswith', 'lunch with', 'receives', ' sign ', ' photo with ', 'meeting ', 'attends ', 'attend ', ' met ', 'participates',
                'participate', 'haslunch ', 'hosts ', 'host ', 'co-host', 'holds ', 'hold ', 'chairs ', 'dinner with', 'audience with ', ' lunch with ',
                ' leads ', ' joins ', ' join ', ' lead ']
        talk = ['a remark ', 'remarks ', 'speech ', 'talk ', 'provides an update ', 'moderates', 'addresses ', 'testifies ', 'speaks ', 'presents ', 'present ',
                'testify ', 'briefs ', 'brief ', 'deliver ', 'delivers ']
        for i, text in enumerate(self.current.text):
    ...
    def add_accom(self):
        ...
        for i, t in enumerate(self.current.text):
            ...
            if 'Biden' in t:
                dp.append('Biden')
            try:
                dp.remove(self.current.lastnames[i])
            except:
                dp
            if len(dp) == 0:
                ACC.append('')
    ...
    def AUTO_COMPLETE(self):
        ...
        self.fixtext('andUK Ministerof StateforMiddle East,North Africaand North AmericaJames Cleverlyin',
                     'and UK Ministerof State for Middle East, North Africa and North America James Cleverly in')
        ...
        self.fixtext('LOCAL Secretary participates in a G7 Family Photo, in London, United Kingdom.',
                     'LOCAL Secretary Blinken participates in a G7 Family Photo, in London, United Kingdom. ')
