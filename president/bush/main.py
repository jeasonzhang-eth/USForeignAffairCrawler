...
def get_month_link_list(browser, ):
    year_link_list = [
    ...
    for year_link in year_link_list:
        ...
        for month in month_list:
            month_link_element = browser.find_element((By.LINK_TEXT, month))
            _month_link_list.append(str(month_link_element.get_attribute('href')))  # 获取每个月的链接
    return _month_link_list


def get_day_link_text(browser, output_file_name, month_link_list):
    ...
    for month_link in month_link_list:
        ...
        if year.__contains__('htm'):
            year = '2004'
        print(year)
        browser.open(month_link)
        day_link_td = browser.find_element((By.XPATH, '/html/body/table[2]/tbody/tr[2]/td[3]'))
...
def main():
    ...
    month_link_list = get_month_link_list(browser, )
    get_day_link_text(browser, output_file_name_bush_link, month_link_list)
